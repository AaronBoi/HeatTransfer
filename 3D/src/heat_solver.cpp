    ...
    for (int x = 0; x < numX; x++) {
        for (int y = 0; y < numY; y++) {
            for (int z = 0; z < numZ; z++) {
                ...
                temp[x][y][z] = temperature[x][y][z] + simdt * (dusimdt + radiationTempChange);
            }
        }
    }
    ...
    for (int x = 0; x < numX; x++) {
        for (int y = 0; y < numY; y++) {
            for (int z = 0; z < numZ; z++) {
                ...
                float T_self = temperature[x][y][z];
                ...
                auto accumulate = [&](int nx, int ny, int nz) {
                    if (isMaterial[nx][ny][nz] != 0) return;
                    float T_amb = temperature[nx][ny][nz];
                ...
                temp[x][y][z] = (thermalMass * T_self + simdt * G_Tamb_sum) / (thermalMass + simdt * G);
            }
        }
    }
    memcpy(temperature, temp, sizeof(temperature));
